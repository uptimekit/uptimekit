...
export async function enforceMonitorQuotaOrThrow(input: {
    ...
    const quotaState = await getOrganizationQuotaState(
        organizationId,
        database,
    );
...
export async function applyOrganizationLimitChanges(input: {
    ...
    const { organizationId, activeMonitorLimit, regionsPerMonitorLimit } =
        input;
    ...
    return database.transaction(async (tx) => {
        ...
        const remainingActiveMonitors = allActiveMonitors.filter(
            (record) =>
                !regionOverflowMonitors.some(
                    (paused) => paused.id === record.id,
                ),
        );
        const activeOverflowCount =
            activeMonitorLimit === null
                ? 0
                : Math.max(
                      remainingActiveMonitors.length - activeMonitorLimit,
                      0,
                  );
