...
export const workersRouter = {
    ...
    create: protectedProcedure
        ...
        .handler(
            async ({
                input,
                context,
            }): Promise<{ worker: Worker; key: string }> => {
                requireInstanceAdmin(context);
                ...
                const remainingWorkers =
                    uniqueNextWorkerIds.size > 0
                        ? await tx
                              ...
                              .from(worker)
                              .where(
                                  inArray(worker.id, [...uniqueNextWorkerIds]),
                              )
                        : [];
                ...
                for (const monitorRecord of affectedMonitors) {
                    ...
                    const nextLocations = [
                        ...new Set(
                            nextWorkerIds
                                .map((workerId) =>
                                    workerLocationMap.get(workerId),
                                )
                                .filter(
                    ...
                    await tx
                        ...
                        .set({
                            ...
                            locations: nextLocations,
                            active: hasAssignedWorkers
                                ? monitorRecord.active
                                : false,
    ...
    listActive: protectedProcedure
        .route({
            ...
            description:
                "List active workers available for monitor assignment.",
        })
        .handler(async () => {
            return db
                .select({
