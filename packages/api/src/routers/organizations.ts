    ...
    getActiveOidcProvider: writeProcedure
        ...
        .handler(async ({ context }) => {
            ...
            const [provider] = await db
                ...
                .where(
                    eq(organizationOidcProvider.organizationId, organizationId),
                )
                .limit(1);
    ...
    upsertActiveOidcProvider: writeProcedure
        ...
        .handler(async ({ context, input }) => {
            ...
            const [existingProvider] = await db
                ...
                .limit(1);
            const clientSecret =
                input.clientSecret ?? existingProvider?.clientSecret;

            if (!clientSecret) {
                throw new ORPCError("BAD_REQUEST", {
                    message:
                        "Client secret is required before enabling OIDC sign-in.",
                });
            }
