    ...
    list: adminProcedure
        .meta({
            openapi: {
                ...
                description: "List all organizations with member and monitor counts. Admin only.",
            },
        })
        .input(
            z
        ...
        .handler(async ({ input }) => {
            ...
            if (input?.plan) {
                filters.push(eq(organization.plan, input.plan));
            }
            ...
            const orgsWithCounts = await db
                .select({
                    ...
                    monitorCount: sql<number>`(SELECT COUNT(*) FROM "monitor" WHERE "monitor"."organization_id" = "organization"."id")`.as("monitor_count"),
                })
                .from(organization)
                .where(whereClause)
    ...
    get: adminProcedure
        .meta({
            openapi: {
                ...
                description: "Get detailed organization information with members. Admin only.",
            },
        })
