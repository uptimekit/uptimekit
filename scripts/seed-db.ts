...
const incidentActivities = [
    ...
    },
    {
        ...
        incidentId: ids.incidents.api,
        message:
            "Error rate is back within normal range. Monitoring continues.",
        type: "comment",
...
async function seedRelationalData(demoUserId: string) {
    await db.transaction(async (tx) => {
        ...
        await tx.insert(statusPage).values({
            ...
            domain: null,
            description:
                "Live uptime and incident history for Acme Cloud services.",
            public: true,
            password: null,
            design: {
                themeId: "signal",
                ...
                events.push({
                    ...
                    statusCode:
                        ...
                        monitorRecord.type === "keyword"
                            ...
                            : null,
                    error:
                        status === "down" ? "Synthetic upstream timeout" : null,
                    location,
                    dnsLookup:
                        status === "down"
                            ? null
                            : Math.max(4, Math.round(latency * 0.08)),
                    tcpConnect:
                        status === "down"
                            ? null
                            : Math.max(6, Math.round(latency * 0.12)),
                    tlsHandshake:
                        status === "down"
                            ? null
                            : Math.max(8, Math.round(latency * 0.18)),
                    ttfb:
                        status === "down"
                            ? null
                            : Math.max(12, Math.round(latency * 0.45)),
                    transfer:
                        status === "down"
                            ? null
                            : Math.max(4, Math.round(latency * 0.17)),
                });
