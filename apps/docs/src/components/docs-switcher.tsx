...
import { Book, ChevronsUpDown, Code, Globe, Webhook } from "lucide-react";
	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				<button
					type="button"
					className="mt-2 flex w-full items-center gap-2 rounded-lg border p-2 text-left font-medium text-sm transition-colors hover:bg-fd-accent hover:text-fd-accent-foreground data-[state=open]:bg-fd-accent data-[state=open]:text-fd-accent-foreground"
				>
					...
					<div className="flex flex-1 flex-col overflow-hidden">
						...
						<span className="mt-1 truncate text-fd-muted-foreground text-xs">
							{current.description}
						</span>
					</div>
