        ...
        <div className="flex min-h-screen flex-col items-center justify-center bg-background font-sans text-foreground">
            <div className="mx-auto max-w-2xl px-4 text-center">
                ...
                <p className="mb-8 text-lg text-muted-foreground">
                    This is the status page domain. To view a specific status
                    page, add the page slug to the URL.
                </p>
                <p className="text-muted-foreground text-sm">
                    Example: {STATUS_PAGE_DOMAIN || "status.example.com"}
                    /your-page-slug
                </p>
            </div>
        </div>
...
export default async function StatusPage() {
    ...
    if (isStatusPageDomain(host)) {
        if (DEFAULT_STATUS_PAGE_SLUG) {
            redirect(`/${DEFAULT_STATUS_PAGE_SLUG}` as never);
        }
