...
function useCreateStatusUpdateFormModel({
    statusPageId,
    open,
    onOpenChange,
    ...
    const { mutate, isPending } = useMutation({
        ...
        onError: (error: Error) => {
            sileo.error({
                title: `Failed to post status update: ${error.message}`,
            });
        },
    });
...
type CreateStatusUpdateFormModel = ReturnType<
    typeof useCreateStatusUpdateFormModel
>;

function CreateStatusUpdateFormContentSection5({
    model,
}: {
    model: CreateStatusUpdateFormModel;
}) {
    const {
        statusPageId,
        open,
...
function CreateStatusUpdateFormDialog4({
    model,
}: {
    model: CreateStatusUpdateFormModel;
}) {
    ...
    return (
        <DialogPanel className="space-y-8">
            ...
            <CreateStatusUpdateFormContentSection5 model={model} />
        </DialogPanel>
    );
}

function CreateStatusUpdateFormDialog3({
    model,
}: {
    model: CreateStatusUpdateFormModel;
}) {
    ...
    return (
        <form onSubmit={form.handleSubmit(onSubmit)} className="contents">
            <CreateStatusUpdateFormDialog4 model={model} />
...
function CreateStatusUpdateFormDialog2({
    model,
}: {
    model: CreateStatusUpdateFormModel;
}) {
    ...
    return (
        <Form {...form}>
            <CreateStatusUpdateFormDialog3 model={model} />
        </Form>
    );
}

function CreateStatusUpdateFormDialog1({
    model,
}: {
    model: CreateStatusUpdateFormModel;
}) {
    ...
    return (
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[700px]">
            ...
            <CreateStatusUpdateFormDialog2 model={model} />
        </DialogContent>
    );
}
function CreateStatusUpdateFormView({
    model,
}: {
    model: CreateStatusUpdateFormModel;
}) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <CreateStatusUpdateFormDialog1 model={model} />
        </Dialog>
    );
}
...
export function CreateStatusUpdateForm({
    ...
    const model = useCreateStatusUpdateFormModel({
        statusPageId,
        open,
        onOpenChange,
    });
    return <CreateStatusUpdateFormView model={model} />;
}
