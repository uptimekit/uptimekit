...
function useCreateIncidentFormModel() {
    ...
    const createIncident = useMutation(
        orpc.incidents.create.mutationOptions({
            ...
            onError: (err) => {
                sileo.error({
                    title: `Failed to create incident: ${err.message}`,
                });
            },
        }),
    );
    ...
    const selectedMonitorIds = new Set(monitorIds);
    const selectedMonitors = monitors.filter((monitor) =>
        selectedMonitorIds.has(monitor.id),
    );

    const selectedStatusPageIds = new Set(statusPageIds);
    const selectedStatusPages = statusPages.filter((statusPage) =>
        selectedStatusPageIds.has(statusPage.id),
    );
    ...
    return {
        ...
        selectedMonitorIds,
        selectedMonitors,
        selectedStatusPageIds,
        selectedStatusPages,
    };
}

type CreateIncidentFormModel = ReturnType<typeof useCreateIncidentFormModel>;

function CreateIncidentFormContentSection2({
    model,
}: {
    model: CreateIncidentFormModel;
}) {
    const {
        router,
        form,
...
function CreateIncidentFormFormSection1({
    model,
}: {
    model: CreateIncidentFormModel;
}) {
    ...
    return (
        ...
        >
            <CreateIncidentFormContentSection2 model={model} />
...
function CreateIncidentFormView({ model }: { model: CreateIncidentFormModel }) {
    const {
        router,
        form,
        severity,
    ...
    return (
        <Form {...form}>
            <CreateIncidentFormFormSection1 model={model} />
        </Form>
    );
}

export function CreateIncidentForm() {
    const model = useCreateIncidentFormModel();
    return <CreateIncidentFormView model={model} />;
}
