...
export function ConfigurationSettingsForm() {
    ...
    const { register, handleSubmit, reset, formState: { isDirty } } = useForm<ConfigFormData>({
        defaultValues: {
            instanceName: "",
    ...
    useEffect(() => {
        if (data?.items) {
            const instanceName = data.items.find((i) => i.key === "instance_name")?.value || "";
            const dataRetentionDays = data.items.find((i) => i.key === "data_retention_days")?.value || "30";
    ...
    const saveMutation = useMutation({
        mutationFn: async (values: ConfigFormData) => {
            await Promise.all([
                client.configuration.set({ key: "instance_name", value: values.instanceName }),
                client.configuration.set({ key: "data_retention_days", value: values.dataRetentionDays }),
            ]);
        },
        onSuccess: () => {
