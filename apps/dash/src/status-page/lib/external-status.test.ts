...
describe("external status helpers", () => {
    ...
    it("maps upstream component statuses to status page statuses", () => {
        ...
        expect(mapExternalComponentStatus("UNDERMAINTENANCE")).toBe(
            "maintenance",
        );
        expect(mapExternalComponentStatus("DEGRADED_PERFORMANCE")).toBe(
            "degraded",
        );
        expect(mapExternalComponentStatus("PARTIAL_OUTAGE")).toBe(
            "partial_outage",
        );
