...
export default async function IncidentDetailsPage({
	...
	const [reports, activeReports, activeMaintenances] = await Promise.all([
		getStatusPageReports(pageConfig.id, 1000), // Increased limit as safety net
		getActiveStatusPageReports(pageConfig.id),
		getActiveMaintenances(pageConfig.id),
	]);

	// Find report in either active or history
	// We search active first, then history
	const reportItem =
	...
	const incidentData = {
		...
		detailsLink: `/incidents/${reportItem.id}`,
	};
